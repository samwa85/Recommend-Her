#!/usr/bin/env node
/**
 * Deploy to recommendher.insforge.site
 */
...
const API_KEY = 'ik_73120ab8bd730f732dd95bbdc954e38a';
const API_BASE_URL = 'https://stz6f3dz.us-east.insforge.app';
  ...
  return new Promise((resolve, reject) => {
    ...
    archive.directory(sourceDir, false, (entry) => {
      ...
      for (const pattern of exclude) {
        if (name.startsWith(pattern + '/') || name === pattern || name.includes('/' + pattern + '/')) {
          return false;
        }
      }
      return entry;
...
async function deploy() {
  log('');
  log('🚀 Deploying to recommendher.insforge.site', 'cyan');
  log('═══════════════════════════════════════════', 'cyan');
  log('');
  ...
  log(`   Live URL: ${result.url || 'https://recommendher.insforge.site'}`, 'cyan');
  log('');
